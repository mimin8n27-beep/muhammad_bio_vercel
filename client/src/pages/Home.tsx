...
export default function Home() {
  ...
  const tools = [
    { name: "n8n", icon: "🔴", description: t("tools.n8n.desc") },
    { name: "Zapier", icon: "⚡", description: t("tools.zapier.desc") },
    { name: "Make", icon: "🟣", description: t("tools.make.desc") },
  ];
  ...
  const pricingPlans = [
    {
      ...
      base: "$150–$400",
      premium: lang === "ar"
        ? ["توثيق PDF: +$50", "فيديو Demo: +$100", "دعم أسبوع: +$50"]
        : ["PDF Documentation: +$50", "Video Demo: +$100", "1 Week Support: +$50"],
      final: "$200–$450",
      hours: "4-8 hours",
      popular: false,
    },
    {
      ...
      base: "$400–$900",
      premium: lang === "ar"
        ? ["توثيق PDF: +$50-$100", "فيديو Demo: +$100-$200", "دعم أسبوعين: +$80-$150"]
        : ["PDF Documentation: +$50–$100", "Video Demo: +$100–$200", "2 Weeks Support: +$80–$150"],
      final: "$500–$1,050",
      hours: "8-15 hours",
      popular: true,
    },
    {
      ...
      base: "$900–$2,500+",
      premium: lang === "ar"
        ? ["توثيق PDF: +$100-$150", "فيديو Demo: +$150-$300", "دعم 3 أسابيع: +$100-$200"]
        : ["PDF Documentation: +$100–$150", "Video Demo: +$150–$300", "3 Weeks Support: +$100–$200"],
      final: "$1,050–$2,800+",
      hours: "15-30 hours",
      popular: false,
    },
    {
      ...
      base: "$2,500+",
      premium: lang === "ar"
        ? ["الباقة الكاملة (PDF + فيديو + دعم)", "الخيارات: +$250-$600+"]
        : ["Full Suite (PDF + Video + Support)", "Options: +$250–$600+"],
      final: "$2,750+",
      hours: "30+ hours",
      popular: false,
    },
    ...
    <div className="min-h-screen bg-white text-foreground" dir={dir}>
        ...
        <div className="container relative z-10">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div className="animate-slideInLeft">
              ...
              <p className="text-lg text-muted-foreground mb-8 leading-relaxed">{t("hero.desc")}</p>
              <div className="flex gap-4 flex-wrap">
                <Button onClick={() => scrollTo("contact-section")} className="bg-primary hover:bg-primary/90 text-white gap-2">
                  {t("hero.cta.contact")} <ArrowRight className="w-4 h-4" />
                </Button>
                <Button onClick={handleWhatsApp} className="bg-green-500 hover:bg-green-600 text-white gap-2">
                  {t("hero.cta.start")} <ArrowRight className="w-4 h-4" />
                </Button>
              </div>
              <div className="mt-12 flex gap-8">
                <div><p className="text-3xl font-bold text-primary">50+</p><p className="text-sm text-muted-foreground">{t("hero.stat.automations")}</p></div>
                <div><p className="text-3xl font-bold text-primary">50+</p><p className="text-sm text-muted-foreground">{t("hero.stat.clients")}</p></div>
                <div><p className="text-3xl font-bold text-primary">24/7</p><p className="text-sm text-muted-foreground">{t("hero.stat.uptime")}</p></div>
              </div>
            </div>
            <div className="animate-slideInRight relative">
              <div className="relative h-96 md:h-full min-h-96 rounded-2xl overflow-hidden shadow-2xl border-4 border-primary/10">
                <img src={PROFILE_IMAGE} alt="Muhammad" className="w-full h-full object-cover" />
      ...
      <section id="expertise-section" className="py-20 md:py-32 bg-white border-t border-border">
        <div className="container">
          <div className="mb-16">
            <h2 className="text-4xl md:text-5xl font-bold text-foreground mb-4">{t("expertise.title")}</h2>
            <p className="text-lg text-muted-foreground max-w-2xl">{t("expertise.desc")}</p>
          </div>
          <div className="grid md:grid-cols-3 gap-8">
            {expertise.map((item, index) => (
              <div key={index} className="group p-8 bg-card rounded-xl border border-border hover:border-primary hover:shadow-lg transition-all duration-300 animate-fadeInUp" style={{ animationDelay: `${index * 100}ms` }}>
      ...
      <section className="py-20 md:py-32 bg-secondary/30 border-t border-border">
        <div className="container">
          <h2 className="text-4xl md:text-5xl font-bold text-foreground mb-16 text-center">{t("tools.title")}</h2>
          <div className="grid md:grid-cols-3 gap-8 mb-16">
            {tools.map((tool, index) => (
              <div key={index} className="p-8 bg-white rounded-xl border border-border hover:border-primary hover:shadow-lg transition-all duration-300 text-center animate-fadeInUp" style={{ animationDelay: `${index * 100}ms` }}>
          ...
          <div className="relative h-96 rounded-2xl overflow-hidden shadow-2xl">
            <img src="https://d2xsxph8kpxj0f.cloudfront.net/310519663184092711/5MnDUtM4VGYpdQp79PzkMf/automation-tools-visual-aci4HV4cLntViBzNTquURM.webp" alt="Automation Tools" className="w-full h-full object-cover" />
          </div>
        </div>
      </section>
          ...
          {projectsLoading ? (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {projects.map((project, index) => (
                <div key={project.id} onClick={() => setSelectedProject(project)} className="group cursor-pointer bg-card rounded-xl border border-border hover:border-primary hover:shadow-lg transition-all duration-300 overflow-hidden animate-fadeInUp" style={{ animationDelay: `${index * 100}ms` }}>
                  <div className="aspect-video bg-secondary overflow-hidden">
                    {project.image_url ? (
                      <img src={project.image_url} alt={project.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center text-muted-foreground">
                        <div className="text-center"><div className="text-4xl mb-2">⚙️</div><p className="text-xs">{t("portfolio.noImage")}</p></div>
                      </div>
                    )}
                  </div>
                  <div className="p-5">
                    ...
                    {project.tools && (
                      <div className="flex flex-wrap gap-2">
                        ...
                        {toolsList(project.tools).length > 4 && <span className="px-2 py-1 bg-secondary rounded-md text-muted-foreground text-xs">+{toolsList(project.tools).length - 4}</span>}
                      </div>
                    )}
                  </div>
                </div>
      ...
      <section id="solutions-section" className="py-20 md:py-32 bg-white border-t border-border">
        <div className="container">
          <h2 className="text-4xl md:text-5xl font-bold text-foreground mb-16">{t("solutions.title")}</h2>
          <div className="grid md:grid-cols-2 gap-8 mb-16">
            {solutions.map((solution, index) => (
              <div key={index} className="p-8 bg-card rounded-xl border border-border hover:border-primary hover:shadow-lg transition-all duration-300 animate-fadeInUp" style={{ animationDelay: `${index * 100}ms` }}>
          ...
          <div className="relative h-96 rounded-2xl overflow-hidden shadow-2xl">
            <img src="https://d2xsxph8kpxj0f.cloudfront.net/310519663184092711/5MnDUtM4VGYpdQp79PzkMf/workflow-process-visual-fQ6qNuSyS9u5e8QcLjyYHh.webp" alt="Workflow" className="w-full h-full object-cover" />
          </div>
        </div>
      </section>
      ...
      <section className="py-20 md:py-32 bg-secondary/30 border-t border-border">
        <div className="container">
          <h2 className="text-4xl md:text-5xl font-bold text-foreground mb-4 text-center">{t("niches.title")}</h2>
          <p className="text-lg text-muted-foreground text-center max-w-2xl mx-auto mb-16">{t("niches.desc")}</p>
          <div className="grid md:grid-cols-3 gap-6">
            {niches.map((niche, index) => (
              <div key={index} className="p-6 bg-white rounded-xl border border-border hover:border-primary hover:shadow-lg transition-all duration-300 flex items-center gap-4 animate-fadeInUp" style={{ animationDelay: `${index * 100}ms` }}>
                <div className="w-12 h-12 bg-primary/10 rounded-lg flex items-center justify-center flex-shrink-0 text-primary">{niche.icon}</div>
        ...
        <div className="container">
          ...
          <div className="grid md:grid-cols-4 gap-6 mb-12">
            {pricingPlans.map((plan, i) => (
              <div key={i} className={`relative rounded-2xl border-2 p-6 flex flex-col transition-all duration-300 group ${plan.popular ? "border-primary shadow-xl shadow-primary/20 bg-white scale-105" : "border-border bg-white hover:border-primary hover:shadow-lg hover:-translate-y-1"}`}>
                {plan.popular && (
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2">
                    <span className="bg-primary text-white text-xs font-bold px-4 py-1.5 rounded-full flex items-center gap-1">{t("pricing.popular")}</span>
                  </div>
                )}
                <h3 className="text-2xl font-bold text-foreground mb-1">{plan.name}</h3>
                <p className="text-sm text-muted-foreground mb-5">{plan.desc}</p>
                <div className="bg-blue-50 rounded-xl p-4 mb-5">
                  <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1 font-semibold">{t("pricing.basePrice")}</p>
                  <p className="text-2xl font-bold text-primary">{plan.base}</p>
                </div>
                <div className="mb-5 flex-1">
                  <p className="text-xs text-muted-foreground uppercase tracking-wider mb-3 font-semibold">{t("pricing.premiumOptions")}</p>
                  <ul className="space-y-1.5">
                    {plan.premium.map((opt, j) => (
                      <li key={j} className="text-sm text-muted-foreground flex items-start gap-2"><span className="text-primary mt-0.5">•</span>{opt}</li>
                    ))}
                  </ul>
                </div>
                <div className="bg-gray-50 rounded-xl p-4 mb-5">
                  <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1 font-semibold">{t("pricing.finalPrice")}</p>
                  <p className="text-xl font-bold text-foreground">{plan.final}</p>
                </div>
                <p className="text-xs text-muted-foreground text-center mb-4 flex items-center justify-center gap-1"><span>🕐</span> {plan.hours}</p>
                <button onClick={() => scrollTo("contact-section")} className={`w-full py-3 rounded-xl font-semibold text-sm transition-all duration-200 ${plan.popular ? "bg-primary text-white hover:bg-primary/90 shadow-md hover:shadow-lg hover:shadow-primary/30" : "border-2 border-border text-foreground hover:border-primary hover:text-primary"}`}>
      ...
      <section className="py-20 md:py-32 bg-white border-t border-border">
        <div className="container max-w-3xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-bold text-foreground mb-4">{t("howIWork.title")}</h2>
          ...
          <div className="relative">
            ...
            {workSteps.map((step, i) => (
              <div key={i} className="flex gap-6 mb-10 group">
                <div className="flex-shrink-0 w-14 h-14 bg-primary text-white rounded-full flex items-center justify-center font-bold text-lg shadow-lg shadow-primary/30 group-hover:scale-110 transition-transform duration-200 z-10">{step.n}</div>
                <div className="flex-1 pb-8 border-b border-border last:border-0 group-hover:translate-x-1 transition-transform duration-200">
                  ...
                  <p className="text-sm text-foreground mt-1"><span className="font-semibold">{t("howIWork.duration")}:</span> {step.duration}</p>
                </div>
              </div>
            ))}
          </div>
          <div className="mt-8 bg-blue-50 rounded-2xl p-8 border border-blue-100">
            <h3 className="text-xl font-bold text-foreground mb-6 text-center">{t("howIWork.why.title")}</h3>
      ...
      <footer className="bg-[#0a0a0a] text-white pt-16 pb-8">
        <div className="container">
          <div className="grid md:grid-cols-3 gap-12 mb-12">
            ...
            <div>
              <h4 className="font-bold mb-4 text-white/80 uppercase text-xs tracking-wider">{t("footer.links")}</h4>
              <ul className="space-y-2.5">
                {[
                  ...
                  <li key={i}><button onClick={link.action} className="text-white/50 hover:text-white text-sm transition-colors">{link.label}</button></li>
                ))}
              </ul>
            </div>
            <div>
              <h4 className="font-bold mb-4 text-white/80 uppercase text-xs tracking-wider">{t("footer.getInTouch")}</h4>
              <ul className="space-y-3">
                <li><a href={`mailto:${EMAIL}`} className="text-white/50 hover:text-white text-sm transition-colors flex items-center gap-2"><MessageSquare className="w-4 h-4" /> {EMAIL}</a></li>
                <li><a href={`https://wa.me/${WHATSAPP_NUMBER.replace(/\D/g, "")}`} target="_blank" rel="noopener noreferrer" className="text-white/50 hover:text-white text-sm transition-colors flex items-center gap-2"><Zap className="w-4 h-4" /> WhatsApp</a></li>
              </ul>
            </div>
          </div>
          <div className="border-t border-white/10 pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
            <p className="text-white/30 text-sm">{t("footer.rights")}</p>
            <p className="text-white/30 text-sm">{t("footer.subtitle")}</p>
          </div>
        </div>
      </footer>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setSelectedProject(null)}>
          <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[92vh] flex flex-col shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="overflow-y-auto flex-1">
              ...
              <div className="p-6" dir={dir}>
                <div className="flex items-start justify-between mb-4">
                  ...
                  <button onClick={() => setSelectedProject(null)} className="p-2 hover:bg-secondary rounded-lg transition-colors flex-shrink-0"><X className="w-5 h-5 text-muted-foreground" /></button>
                </div>
            ...
            <div className="p-4 border-t border-border bg-white rounded-b-2xl flex-shrink-0" dir={dir}>
              <a href={`/portfolio?open=${selectedProject.id}`} className="flex items-center gap-2 w-full justify-center py-3 bg-primary hover:bg-primary/90 rounded-xl font-semibold text-white transition-colors">
